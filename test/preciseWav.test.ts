...
describe('afconvert 参数与文件名约束', () => {
  test('位深映射保持无损：16→LEI16、24→LEI24、32→LEI32、异常回退 LEI16', () => {
    ...
    // Float32 cannot preserve 25-32 bit integer samples.
...
describe('precise-wav 路由', () => {
  ...
  beforeAll(async () => {
    ...
    installPreciseWavRoutes(app, {
      ...
      convert: async (flacPath, wavPath, dataFormat) => {
        ...
        if (dataFormat === 'LEI32') {
          // Simulate afconvert leaving a partial WAV.
          await fsp.writeFile(wavPath, '半成品');
          throw new Error('模拟转换失败');
        }
        const flacBytes = await fsp.readFile(flacPath);
        // Echo the POST body to verify the converter input.
        const wavBytes = new Uint8Array(4 + flacBytes.byteLength);
        wavBytes.set(new TextEncoder().encode('RIFF'));
        wavBytes.set(flacBytes, 4);
        await fsp.writeFile(wavPath, wavBytes);
      },
    });
  ...
  test('POST 流式落盘转换，GET 全量与 Range 均可读', async () => {
    ...
    const firstConvertCall = convertCalls[0];
    if (!firstConvertCall) throw new Error('Expected a conversion call');
    expect(firstConvertCall.dataFormat).toBe('LEI24');
    ...
    // Remove the intermediate FLAC after conversion.
  ...
  test('超过大小上限的上传直接 413 拒绝', async () => {
    ...
    const oversize = await fetch(
      `http://127.0.0.1:${address.port}/precise-wav/45`,
  ...
  test('Windows/Linux 明确返回 501，让播放器使用已有回退路径', async () => {
    ...
    const response = await fetch(
      `http://127.0.0.1:${address.port}/precise-wav/47`,
